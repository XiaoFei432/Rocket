...
import org.apache.openwhisk.core.rocket.{
  RocketAnnotations,
  RocketConfig,
  RocketContainerState,
  RocketCoordinator,
  RocketWarming
}
...
class FunctionPullingContainerPool(
  ...
  def receive: Receive = {
      ...
      if (shuttingDown) {
        ...
        action.toExecutableWhiskAction match {
          case Some(executable) =>
            val rocketState = targetRocketState(create, executable)
            ...
            handleChosenContainer(create, executable, createdContainer, rocketState)
          case None =>
            val message =
    ...
    case ResumeFailed(data) =>
      ...
      creationMessages.remove(sender()).foreach { msg =>
        ...
        handleChosenContainer(msg, data.action, container, targetRocketState(msg, data.action))
      }
  ...
  private def handleChosenContainer(create: ContainerCreationMessage,
                                    ...
                                    rocketState: RocketContainerState) = {
    container match {
  ...
  private def targetRocketState(create: ContainerCreationMessage, executable: ExecutableWhiskAction): RocketContainerState = {
    if (rocketConfig.enabled && RocketAnnotations.isRocket(executable)) {
      val now = System.currentTimeMillis()
      RocketAnnotations.spec(executable).foreach { spec =>
        rocketCoordinator.register(spec)
        rocketCoordinator.observeInvocation(spec.id, now)
      }
      val fromMessage = RocketContainerState.fromName(create.rocketState)
      if (fromMessage != RocketWarming) fromMessage
      else rocketCoordinator.targetState(executable.fullyQualifiedName(false).asString, now)
    } else {
      RocketWarming
    }
  }
