...
import org.apache.openwhisk.core.rocket.{RocketAnnotations, RocketConfig, RocketWarming}
...
class ContainerProxy(factory: (TransactionId,
  ...
  val rocketConfig = Try(loadConfigOrThrow[RocketConfig](ConfigKeys.rocket)).getOrElse(RocketConfig())
    ...
    val activation: Future[WhiskActivation] = initialize
      .flatMap { initInterval =>
        ...
        val rocketEnvironment =
          if (rocketConfig.enabled && RocketAnnotations.isRocket(job.action)) {
            RocketAnnotations.invokeEnvironment(job.action, RocketWarming)
          } else {
