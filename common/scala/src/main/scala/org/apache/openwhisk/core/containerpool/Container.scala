...
import org.apache.openwhisk.core.rocket.{RocketAnnotations, RocketContainerState}
...
trait Container {
  ...
  def run(parameters: JsValue,
    ...
    val rocketMetadata = RocketAnnotations.requestMetadata(environment.fields)
    val visibleParameters = parameters match {
      case obj: JsObject if rocketMetadata.nonEmpty => JsObject(obj.fields ++ rocketMetadata)
      case _                                        => parameters
    }
    val parameterWrapper = JsObject("value" -> visibleParameters)
  ...
  /** Drives a Rocket-aware action runtime to pre-load or off-load ML artifacts. */
  def preload(action: org.apache.openwhisk.core.entity.ExecutableWhiskAction,
              target: RocketContainerState,
              timeout: FiniteDuration,
              maxResponse: ByteSize,
              truncation: ByteSize)(implicit transid: TransactionId): Future[(Interval, ActivationResponse)] = {
    val start =
      transid.started(
        this,
        LoggingMarkers.INVOKER_ACTIVATION_RUN,
        s"sending Rocket pre-load request ${target.name} to $id $addr",
        logLevel = InfoLevel)

    val body = RocketAnnotations.preloadRequest(action, target)
    callContainer("/run", body, timeout, 1, maxResponse, truncation, retry = false, reschedule = false)
      .andThen {
        case Success(r: RunResult) =>
          transid.finished(
            this,
            start.copy(start = r.interval.start),
            s"Rocket pre-load result: ${r.toBriefString}",
            endTime = r.interval.end,
            logLevel = InfoLevel)
        case Failure(t) =>
          transid.failed(this, start, s"Rocket pre-load failed with $t")
      }
      .map { result =>
        val response =
