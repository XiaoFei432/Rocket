...
import org.apache.openwhisk.core.rocket.{RocketAnnotations, RocketConfig, RocketContainerState, RocketWarming}
...
case class Initialize(invocationNamespace: String,
                      ...
                      rocketState: RocketContainerState = RocketWarming)
...
case class RocketPreloadCompleted(data: WarmData)
...
class FunctionPullingContainerProxy(
  ...
  private var pendingRocketState: RocketContainerState = RocketWarming
  ...
  when(Uninitialized) {
    ...
    case Event(job: Initialize, _) =>
      pendingRocketState = job.rocketState
  ...
  when(ClientCreated) {
    ...
    case Event(initializedData: InitializedData, _) =>
      if (rocketConfig.enabled && RocketAnnotations.isRocket(initializedData.action) && pendingRocketState.level > RocketWarming.level) {
        initializeAndPreload(initializedData, pendingRocketState).pipeTo(self)
      } else {
    ...
    case Event(RocketPreloadCompleted(warmData), _) =>
      context.parent ! Resumed(warmData)
      warmData.clientProxy ! ContainerWarmed
      warmData.clientProxy ! RequestActivation()
      startTimerWithFixedDelay(PingCacheName, PingCache, pingCacheInterval)
      startSingleTimer(UnusedTimeoutName, StateTimeout, unusedTimeout)
      goto(Running) using warmData
  ...
  private def initializeAndPreload(data: InitializedData, target: RocketContainerState): Future[RocketPreloadCompleted] = {
    implicit val tid: TransactionId = TransactionId.invokerWarmup
    val timeout = data.action.limits.timeout.duration
    val owEnv = RocketAnnotations
      .invokeEnvironment(data.action, target)
      .map {
        case (key, value: JsString)  => "__OW_" + key.toUpperCase -> value
        case (key, JsNull)           => "__OW_" + key.toUpperCase -> JsString.empty
        case (key, JsBoolean(value)) => "__OW_" + key.toUpperCase -> JsString(value.toString)
        case (key, JsNumber(value))  => "__OW_" + key.toUpperCase -> JsString(value.toString)
        case (key, value)            => "__OW_" + key.toUpperCase -> JsString(value.compactPrint)
      }
    logging.info(
      this,
      s"Rocket pre-loading container ${data.container.containerId.asString} for ${data.action.fullyQualifiedName(false)} to ${target.name}")

    data.container
      .initialize(
        data.action.containerInitializer(owEnv),
        timeout,
        data.action.limits.concurrency.maxConcurrent,
        Some(data.action.toWhiskAction))
      .flatMap { _ =>
        data.container.preload(
          data.action,
          target,
          timeout,
      ...
      .flatMap {
        case (_, response) if response.isSuccess =>
          Future.successful(())
        case (_, response) =>
          Future.failed(new IllegalStateException(s"Rocket pre-load failed: $response"))
      }
      .map(_ =>
        RocketPreloadCompleted(
          WarmData(data.container, data.invocationNamespace, data.action, data.action.rev, Instant.now, data.clientProxy)))
  }
  ...
  private def initializeAndRunActivation(
    ...
    val activation: Future[WhiskActivation] = initialize
      .flatMap { initInterval =>
        ...
        val rocketEnvironment =
          if (rocketConfig.enabled && RocketAnnotations.isRocket(action)) RocketAnnotations.invokeEnvironment(action, pendingRocketState)
          else Map.empty[String, JsValue]
        val env = authEnvironment ++ environment ++ rocketEnvironment ++ Map(
...
object FunctionPullingContainerProxy {
  ...
  def props(factory: (TransactionId,
            ...
            poolConfig: ContainerPoolConfig,
            timeoutConfig: ContainerProxyTimeoutConfig,
            rocketConfig: RocketConfig = Try(loadConfigOrThrow[RocketConfig](ConfigKeys.rocket)).getOrElse(RocketConfig()),
