...
object ContainerCreationMessage extends DefaultJsonProtocol {
  ...
  implicit val serdes = new RootJsonFormat[ContainerCreationMessage] {
    override def write(m: ContainerCreationMessage): JsValue =
      JsObject(
        "transid" -> m.transid.toJson,
        "invocationNamespace" -> JsString(m.invocationNamespace),
        "action" -> m.action.toJson,
        "revision" -> m.revision.toJson,
        "whiskActionMetaData" -> WhiskActionMetaData.serdes.write(m.whiskActionMetaData),
        "rootSchedulerIndex" -> m.rootSchedulerIndex.toJson,
        "schedulerHost" -> JsString(m.schedulerHost),
        "rpcPort" -> JsNumber(m.rpcPort),
        "retryCount" -> JsNumber(m.retryCount),
        "creationId" -> m.creationId.toJson,
        "rocketState" -> JsString(m.rocketState))

    override def read(json: JsValue): ContainerCreationMessage = {
      val fields = json.asJsObject.fields
      ContainerCreationMessage(
        fields("transid").convertTo[TransactionId],
        fields("invocationNamespace").convertTo[String],
        fields("action").convertTo[FullyQualifiedEntityName],
        fields("revision").convertTo[DocRevision],
        WhiskActionMetaData.serdes.read(fields("whiskActionMetaData")),
        fields("rootSchedulerIndex").convertTo[SchedulerInstanceId],
        fields("schedulerHost").convertTo[String],
        fields("rpcPort").convertTo[Int],
        fields.get("retryCount").map(_.convertTo[Int]).getOrElse(0),
        fields.get("creationId").map(_.convertTo[CreationId]).getOrElse(CreationId.generate()),
        fields.get("rocketState").map(_.convertTo[String]).getOrElse("warming"))
    }
  }
}
...
object ContainerCreationAckMessage extends DefaultJsonProtocol {
  ...
  private implicit val fqnSerdes = FullyQualifiedEntityName.serdes
  private implicit val byteSizeSerdes = size.serdes
  implicit val serdes = new RootJsonFormat[ContainerCreationAckMessage] {
    override def write(m: ContainerCreationAckMessage): JsValue =
      JsObject(
        "transid" -> m.transid.toJson,
        "creationId" -> m.creationId.toJson,
        "invocationNamespace" -> JsString(m.invocationNamespace),
        "action" -> m.action.toJson,
        "revision" -> m.revision.toJson,
        "actionMetaData" -> WhiskActionMetaData.serdes.write(m.actionMetaData),
        "rootInvokerIndex" -> m.rootInvokerIndex.toJson,
        "schedulerHost" -> JsString(m.schedulerHost),
        "rpcPort" -> JsNumber(m.rpcPort),
        "retryCount" -> JsNumber(m.retryCount),
        "error" -> m.error.toJson,
        "reason" -> m.reason.toJson,
        "rocketState" -> JsString(m.rocketState))

    override def read(json: JsValue): ContainerCreationAckMessage = {
      val fields = json.asJsObject.fields
      ContainerCreationAckMessage(
        fields("transid").convertTo[TransactionId],
        fields("creationId").convertTo[CreationId],
        fields("invocationNamespace").convertTo[String],
        fields("action").convertTo[FullyQualifiedEntityName],
        fields("revision").convertTo[DocRevision],
        WhiskActionMetaData.serdes.read(fields("actionMetaData")),
        fields("rootInvokerIndex").convertTo[InvokerInstanceId],
        fields("schedulerHost").convertTo[String],
        fields("rpcPort").convertTo[Int],
        fields.get("retryCount").map(_.convertTo[Int]).getOrElse(0),
        fields.get("error").map(_.convertTo[Option[ContainerCreationError]]).getOrElse(None),
        fields.get("reason").map(_.convertTo[Option[String]]).getOrElse(None),
        fields.get("rocketState").map(_.convertTo[String]).getOrElse("warming"))
    }
  }
}
