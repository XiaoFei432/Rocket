...
import org.apache.openwhisk.core.rocket.{RocketAnnotations, RocketConfig, RocketCoordinator}
...
class ContainerManager(jobManagerFactory: ActorRefFactory => ActorRef,
  ...
  private def sendCreationContainerToInvoker(producer: MessageProducer,
    ...
    ContainerManager.observeRocketContainer(invoker, msg)
...
object ContainerManager {
  ...
  private val rocketConfig = Try(loadConfigOrThrow[RocketConfig](ConfigKeys.rocket)).getOrElse(RocketConfig())
  private val rocketCoordinator = new RocketCoordinator(rocketConfig)
  ...
  private[container] def observeRocketContainer(invoker: Int, msg: ContainerCreationMessage): Unit =
    if (rocketConfig.enabled && RocketAnnotations.isRocket(msg.whiskActionMetaData)) {
      RocketAnnotations.spec(msg.whiskActionMetaData).foreach { spec =>
        rocketCoordinator.register(spec)
        rocketCoordinator.observeContainer(invoker, spec.id)
      }
    }
    ...
    if (candidates.isEmpty) {
      ...
      val rocketChoice =
        if (rocketConfig.enabled && RocketAnnotations.isRocket(msg.whiskActionMetaData)) {
          ...
          val functionId = msg.whiskActionMetaData.fullyQualifiedName(false).asString
          RocketAnnotations.spec(msg.whiskActionMetaData).foreach { spec =>
            rocketCoordinator.register(spec)
            rocketCoordinator.observeInvocation(spec.id, now)
          }
          rocketCoordinator
            .chooseInvoker(
              functionId,
              candidates.map(_.id.instance).toVector,
              invoker =>
                candidates.exists(c => c.id.instance == invoker && c.id.userMemory.toMB >= requiredMemory.megabytes),
              now)
            .flatMap(choice => candidates.find(_.id.instance == choice.invokerId))
            .map { instance =>
              val target = rocketCoordinator.targetState(functionId, now)
              logging.info(
                this,
                s"Rocket schedules ${msg.invocationNamespace}/${msg.action} to invoker ${instance.id.instance} with target state ${target.name}")
              ScheduledPair(msg.copy(rocketState = target.name), invokerId = Some(instance.id))
            }
        } else {
          None
        }
      rocketChoice.getOrElse {
