...
class CreationJobManager(feedFactory: (ActorRefFactory, String, String, Int, Array[Byte] => Future[Unit]) => ActorRef,
  ...
  override def receive: Receive = {
    case RegisterCreationJob(msg: ContainerCreationMessage) =>
      val isBlackboxInvocation = msg.whiskActionMetaData.toExecutableWhiskAction.exists(a => a.exec.pull)
      registerJob(msg.invocationNamespace, msg.action, msg.revision, msg.creationId, isBlackboxInvocation)
